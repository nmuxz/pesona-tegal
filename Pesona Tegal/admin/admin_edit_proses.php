...
if (!empty($_FILES['foto_utama']['name'])) {
    ...
    $foto_nama = basename($_FILES['foto_utama']['name']);
    $targetFile = $targetDir . $foto_nama;
    ...
    if (move_uploaded_file($_FILES['foto_utama']['tmp_name'], $targetFile)) {
        ...
        if ($foto_utama_lama && file_exists($foto_utama_lama)) {
            unlink($foto_utama_lama);
        }
        $foto_utama = $targetFile;
    } else {
